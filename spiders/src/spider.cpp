...
void godot::SpiderDuo::interact_bird()
{
    Node* birdNode = get_parent()->get_node_or_null("ptasznik");
    if (!birdNode) return;

    AnimatedSprite2D* bird = Object::cast_to<AnimatedSprite2D>(birdNode);

    Node* colliderNode = bird->get_node_or_null("Area2D");
    if (!colliderNode) return;

    Node* spiderCollider1 = pUrsula->get_node_or_null("Area2D");
    Node* spiderCollider2 = pMartin->get_node_or_null("Area2D");

    Area2D* birdCollider   = Object::cast_to<Area2D>(colliderNode);
    Area2D* martinCollider = Object::cast_to<Area2D>(spiderCollider2);
    Area2D* ursulaCollider = Object::cast_to<Area2D>(spiderCollider1);

    if (!birdCollider || !martinCollider || !ursulaCollider) return;

    Array overlaps = birdCollider->get_overlapping_areas();

    for (int i = 0; i < overlaps.size(); i++) {
        Area2D* area = Object::cast_to<Area2D>(overlaps[i]);

        if (area == martinCollider || area == ursulaCollider) {
            takeDamage(33);
            return;
        }
    }
    ...
    void godot::Enemy::_draw()
    {
        float health_ratio = health / 50.0f;

        Vector2 bar_pos = get_position() + Vector2(-10, -8);
        Vector2 bar_size = Vector2(20, 2);
